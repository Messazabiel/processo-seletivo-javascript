...
Contexto:
Você receberá uma array de objetos que representam diferentes modelos de carros. Cada objeto possui quatro propriedades: marca, modelo, preco e quilometragem.

Tarefas:
Retornar todos os carros que têm um preço maior que 40 mil reais.
Retornar todos os carros que têm uma quilometragem inferior a 15 mil km.
Retornar todos os carros que têm a mesma marca especificada.
Retornar todos os carros que têm o mesmo modelo especificado.
Resultado Esperado:
Crie um novo array contendo apenas os carros que atendem a pelo menos um dos critérios mencionados acima. Não é necessário exibir ou imprimir os resultados, apenas preparar o array filtrado conforme especificado.
...
// Retornar todos os carros que têm um preço maior que 40 mil reais.
function carrosPrecoMaiorQue40Mil(carros) {
    return carros.filter(carro => carro.preco > 40000);
}

// Retornar todos os carros que têm uma quilometragem inferior a 15 mil km.
function carrosQuilometragemInferior15Mil(carros) {
    return carros.filter(carro => carro.quilometragem < 15000);
}

//Retornar todos os carros que têm a mesma marca especificada.
function carrosMesmaMarcaEspecifica(carros, marca) {
    return carros.filter(carro => carro.marca === marca);
}

// Retornar todos os carros que têm o mesmo modelo especificado.
function carrosMesmoModeloEspecifico(carros, modelo) {
    return carros.filter(carro => carro.modelo === modelo);
}


console.log("Carros com preço maior que 40 mil reais:");
console.log(carrosPrecoMaiorQue40Mil(carros));
console.log("\nCarros com quilometragem inferior a 15 mil km:");
console.log(carrosQuilometragemInferior15Mil(carros));
console.log("\nCarros da marca Toyota:");
console.log(carrosMesmaMarcaEspecifica(carros, "Toyota"));
console.log("\nCarros do modelo Corolla:");
console.log(carrosMesmoModeloEspecifico(carros, "Corolla"));
